...
--
-- |intervals| must expose `id INT64`, `ts INT64`, `dur INT64` (>= 0).
-- Input need not be sorted. `id` is carried through as opaque payload: any
-- int64 value (including negatives) is legal and duplicates behave as
-- independent intervals, exactly like the SQL macro this replaces.
-- Output:
...
(
  ...
  FROM __intrinsic_table_ptr(
    __intrinsic_interval_self_intersect(
      (
        SELECT
          __intrinsic_isi_plain_intervals_agg(
