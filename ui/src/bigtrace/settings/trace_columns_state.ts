...
class TraceColumnsState {
  ...
  // The user's explicit selection, or `null` to mean "fall back to schema
  // defaults". A persisted empty array is treated as the null default (a
  // zero-column grid is never what the user wants).
  ...
  // Revert to the schema's defaults.
  clear(): void {
