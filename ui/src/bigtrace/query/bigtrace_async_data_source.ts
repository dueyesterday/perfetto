...
export class BigtraceAsyncDataSource implements DataSource {
  ...
  // declared on the last successful fetch. Empty array until a fetch
  // lands. Used by the results page to populate its column picker.
  ...
  useRows(_model: DataSourceModel): DataSourceRows {
    ...
    // FlatModel exposes columns the grid is currently displaying.
    // Map back to field names for the wire — same alias→field
    // remap the order/filter helpers do, only here we project not
    // sort/select.
  ...
  private async fetchMoreRows(offset: number, limit: number) {
    ...
    try {
      ...
      // Always overwrite — `availableColumns` reflects the
      // backend's current view of what's projectable. A change here
      // (e.g. the sidecar was just dropped by a TTL sweep) needs to
      // propagate to the picker.
