      ...
      callback: async () => {
        ...
        await addDebugSliceTrack({
          ...
          data: {
            ...
            //
            // `android_lock_held.dur` is how long the lock was actually held.
            // Every track here shows a single lock, so a hold is cut short
            // where the next thread takes that lock, leaving one
            // non-overlapping timeline per track. The last hold of a lock has
            // no successor, hence LEAD's fallback to the hold's own end:
            // without it scalar MIN(NULL, x) is NULL, which the debug track
            // maps to dur = -1 and draws as an unfinished slice.
            sqlSource: `
                    SELECT
                      id AS slice_id,
                      'slice' AS table_name,
                      ...
                      ts,
                      MIN(
                        LEAD(ts, 1, ts + dur) OVER (
                          PARTITION BY lock_name ORDER BY ts),
                        ts + dur) - ts AS dur
