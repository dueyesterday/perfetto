...
export class MaterializationService {
  ...
  private async materializeEntries(entries: IrEntry[]): Promise<void> {
    ...
    const needsIntervalIntersect = entries.some((e) =>
      e.sql.includes('_interval_intersect!'),
    );
    if (needsIntervalIntersect) {
      await this.engine.query('INCLUDE PERFETTO MODULE intervals.intersect');
    }
