...
import {joinConditionToSql, getJoinColumnAliases} from './join';
...
function tryFold(stmt: SqlStatement, node: NodeData): boolean {
  switch (node.type) {
    ...
    case 'distinct': {
      if (stmt.distinct) return false;
      const selectedCols = Object.entries(node.distinctColumns)
        .filter(([_, checked]) => checked)
        .map(([col]) => col);
      if (selectedCols.length > 0 && stmt.columns === '*') {
        stmt.columns = selectedCols.join(', ');
      }
      stmt.distinct = true;
      return true;
    }
    case 'extract_arg': {
      ...
      const exprParts = node.extractions
        ...
        .map(
          (e) => `extract_arg(${e.column}, '${e.argName}') AS ${e.alias}`,
        );
...
export function buildIR(
  ...
  function emitEntry(sql: string, deps: string[]): string {
    const hash = irHash(sql, deps);
    if (!entryByHash.has(hash)) {
      entryByHash.add(hash);
      entries.push({hash, sql, deps: [...deps]});
    }
    return hash;
  }
  ...
  function emitCurrent() {
    if (current && currentTailId) {
      ...
      const hash = emitEntry(sql, currentDeps);
  ...
  for (const n of order) {
    ...
    // Group-related nodes: skip in IR (no SQL generation yet).
    if (
      n.type === 'group' ||
      n.type === 'group_input' ||
      n.type === 'group_output'
    ...
    if (n.type === 'join') {
      ...
      const condition = joinConditionToSql(n);
      ...
      const rightAvail = rightInput
        ? getOutputColumnsForNode(nodes, connections, rightInput.id, sqlModules)
        : undefined;
      const leftNames = (leftAvail ?? []).map((c) => c.name);
      const rightNames = (rightAvail ?? []).map((c) => c.name);
      const aliases = getJoinColumnAliases(n, leftNames, rightNames);
      const selectCols: string[] = [];
      for (const a of aliases) {
        const alias = a.side === 'left' ? 'l' : 'r';
        const expr = `${alias}.${a.column}`;
        selectCols.push(a.alias !== a.column ? `${expr} AS ${a.alias}` : expr);
      }
      let selectClause: string;
      if (selectCols.length > 1) {
        selectClause = '\n  ' + selectCols.join(',\n  ');
      } else if (selectCols.length === 1) {
        selectClause = selectCols[0];
      } else {
        selectClause = '*';
      }

      const sql = `SELECT ${selectClause}\nFROM ${leftRef} AS l\n${n.joinType} JOIN ${rightRef} AS r ${condition}`;
    ...
    if (n.type === 'interval_intersect') {
      ...
      const partitionCols = Object.entries(n.partitionColumns)
        .filter(([_, checked]) => checked)
        .map(([col]) => col);
