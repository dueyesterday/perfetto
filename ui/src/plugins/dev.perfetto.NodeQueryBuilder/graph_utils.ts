...
import {getJoinColumnAliases} from './join';
...
export function isNodeValid(
  ...
  switch (node.type) {
    ...
    case 'join': {
      ...
      const hasCondition =
        node.joinConditionType === 'USING'
          ? node.joinUsing !== ''
          : node.joinOn !== '';
      return hasLeft && hasRight && hasCondition;
    }
    case 'extend': {
      const inputs = findConnectedInputs(nodes, connections, node.id);
      const hasDockedParent = findDockedParent(nodes, node.id) !== undefined;
      const hasLeft = hasDockedParent || inputs.has(0);
      const hasRight = inputs.has(1);
      return hasLeft && hasRight && node.leftColumn !== '' && node.rightColumn !== '';
    }
    ...
    case 'sort':
      return (node.conditions ?? []).some((c) => c.column !== '') ||
        node.sortColumn !== '';
    case 'limit':
...
// Returns undefined if columns can't be determined (e.g. after a join).
export function getOutputColumnsForNode(
  ...
  for (const n of order) {
    switch (n.type) {
      ...
      case 'join': {
        // Merge selected columns from both sides, using resolved aliases.
        ...
        const joinAliases = getJoinColumnAliases(n, leftNames, rightNames);
        const joinResult: ColumnDef[] = joinAliases.map((a) => {
          const source =
            a.side === 'left' ? leftAvail : rightAvail;
          const orig = source?.find((c) => c.name === a.column);
          return {name: a.alias, type: orig?.type};
        });
        columns = joinResult.length > 0 ? joinResult : undefined;
        break;
      }
      case 'extend': {
        ...
        const extendResult: ColumnDef[] = [
          ...(leftAvail ?? []),
          ...extendAliases.map((a) => ({name: a.alias})),
        ];
      ...
      case 'distinct': {
        const selected: ColumnDef[] = Object.entries(n.distinctColumns)
          .filter(([_, checked]) => checked)
          .map(
            ([col]) => columns?.find((c) => c.name === col) ?? {name: col},
          );
        if (selected.length > 0) {
          columns = selected;
        }
...
export function getIntervalIntersectOutputColumns(
  ...
  for (const [col, checked] of Object.entries(node.partitionColumns)) {
    if (checked) {
...
export function collectUpstream(
  ...
  // Visit secondary input (join/interval_intersect/union_all right side, port 1)
  if (
    node.type === 'join' ||
    node.type === 'extend' ||
