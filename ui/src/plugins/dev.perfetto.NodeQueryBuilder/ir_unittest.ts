...
describe('buildIR', () => {
  ...
  it('handles join nodes with two inputs via connections', () => {
    const fromA: NodeData = {
      type: 'from',
      id: 'fA',
      x: 0,
      y: 0,
    ...
    const connections: Connection[] = [
      {fromNode: 'fA', fromPort: 0, toNode: 'j1', toPort: 0},
      {fromNode: 'fB', fromPort: 0, toNode: 'j1', toPort: 1},
    ];
    const {nodes} = makeStore([fromA, fromB, join]);
    const entries = buildIR(nodes, connections, 'j1', undefined);
    expect(entries).toBeDefined();
    expect(entries!.length).toBe(1);
    expect(entries![0].sql).toContain('LEFT JOIN thread AS r');
    expect(entries![0].sql).toContain('FROM slice AS l');
    expect(entries![0].sql).toContain('ON l.utid = r.utid');
    // No deps since both inputs are from nodes
    expect(entries![0].deps.length).toBe(0);
  });
