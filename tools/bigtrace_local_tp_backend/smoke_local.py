...
def main() -> int:
    ...
    server = subprocess.Popen(
        [venv_python, os.path.join(HERE, 'server.py'),
         ...
         '--with-datasette', str(DATASETTE_PORT),
         '--log-level', 'warning'],
    ...
    try:
        ...
        # are not materialized so there's no fetchable table. The UI
        # tolerates this by re-running the query if the user clicks
        # Open on a sync history entry.
        sync_code, sync_body = http_status_and_body(
            'GET',
            f'/query_executions/{sync_uuid}:fetch_results?limit=10&offset=0',
        )
        assert sync_code == 404, (
            f'sync :fetch_results expected 404, got {sync_code}: {sync_body}'
        )
        ...
        src_trace = next(
            (os.path.join(traces_dir, n) for n in os.listdir(traces_dir)
        ...
        _, sync_alt = http(
            ...
            body={
                'limit': 5,
                'perfetto_sql': 'SELECT name FROM slice LIMIT 3',
                'settings': [
                    {'setting_id': 'trace_directory', 'values': [alt_dir],
        ...
        print(f'    custom trace_directory={alt_dir!r} -> {len(rows_alt)} rows from {seen_tids}')
        ...
        code, body_text = http_status_and_body(
            ...
            body={
                ...
                'settings': [
                    {'setting_id': 'trace_directory', 'values': [bogus],
                     'category': 'TRACE_ADDRESS'},
                ],
            },
        )
        ...
        print(f'    status={st["status"]} processedTraces={st["processedTraces"]}/{st["totalTraces"]} '
        ...
        _, page = http('GET',
                       f'/query_executions/{async_uuid}:fetch_results?limit=5&offset=0')
        ...
        while time.time() < stream_deadline:
            _, sst_intermediate = http(
                'GET', f'/query_executions/{stream_uuid}:status',
            )
        ...
        print(
            f'    final status={sst_final["status"]} processedTraces='
        ...
        if not observed_inflight_progress:
            ...
            print(
                '    NOTE: no intermediate progress snapshot captured '
                '(traces faster than poll cadence) — streaming not asserted'
            )
        else:
            print(
                f'    observed mid-flight progress; mid-flight '
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{fail_uuid}:status')[1]['status']
                    in ('FAILED', 'SUCCESS', 'CANCELLED'),
            timeout=60.0, label='failed query terminal',
        )
        ...
        # FAILED queries clear tableName/Link, so :fetch_results must
        # return 404. Belt-and-braces with the UI's tableName guard.
        ...
        fcode, fbody = http_status_and_body(
            'GET', f'/query_executions/{fail_uuid}:fetch_results?limit=10&offset=0',
        )
        assert fcode == 404, (
            f'FAILED :fetch_results expected 404, got {fcode}: {fbody}'
        )
        print(f'    FAILED fetch_results=404 (tableName cleared on failure)')
        ...
        _, csub = http('POST', '/execute_bigtrace_query_async',
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{cancel_uuid}:status')[1]['status']
                    in ('CANCELLED', 'SUCCESS', 'FAILED'),
            timeout=120.0, label='cancelled query terminal',
        )
        ...
        _, psub = http('POST', '/execute_bigtrace_query_async',
                       body={'limit': 100_000,
                             'perfetto_sql':
        ...
        while time.time() < partials_deadline:
            _, ps = http(
                'GET', f'/query_executions/{partial_uuid}:status',
            )
            ps_status = ps['status']
        ...
        if not saw_progress:
            ...
            code = http_status(
                'POST', f'/query_executions/{partial_uuid}:cancel'
            )
            assert code == 200, f'cancel returned {code}'
            wait_until(
                lambda: http(
                    'GET', f'/query_executions/{partial_uuid}:status'
                )[1]['status'] in ('CANCELLED', 'SUCCESS', 'FAILED'),
                timeout=120.0, label='partials cancel terminal',
            )
        ...
        wait_until(
            ...
            timeout=60.0, label='long-sql query terminal',
        )
        _, lst3 = http('GET', '/query_executions')
        listed = next(q for q in lst3['queryExecutions']
                      if q['queryUuid'] == long_uuid)
        ...
        assert len(lst2.get('queryExecutions', [])) == n_before - 1, (
            'soft-deleted row still visible in list'
        )
        ...
        wait_until(
            lambda: http(
                'GET', f'/query_executions/{ttl_uuid}:status'
            )[1]['status'] in ('SUCCESS', 'FAILED'),
            timeout=30.0, label='TTL test query terminal',
        )
        ...
        assert page_pre.get('rows'), (
            f'pre-TTL fetch returned no rows: {page_pre}'
        )
        ...
        time.sleep(7)
        # Post-TTL: fetch must 404, tableName/Link must be None,
        ...
        assert ttl_code == 404, (
            f'post-TTL fetch expected 404, got {ttl_code}: {ttl_body}'
        )
        ...
        print(f'    TTL cleared {ttl_uuid[:8]}... fetch=404, tableName=None, status preserved')
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{gl_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=60.0, label='global-limit query terminal',
        )
        ...
        _, gl_page = http(
            'GET', f'/query_executions/{gl_uuid}:fetch_results?limit=100&offset=0',
        )
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{tl_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=60.0, label='trace_limit query terminal',
        )
        ...
        EXPECTED_STATUS_KEYS = {
            'queryUuid', 'status',
            'processedTraces', 'totalTraces', 'processedRows',
        }
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{ss_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=30.0, label=':status-shape FAILED query terminal',
        )
        ...
        if dl_st['status'] == 'IN_PROGRESS':
            dl_code, dl_body = http_status_and_body(
                'DELETE', f'/query_executions/{dl_uuid}',
            )
            assert dl_code == 409, (
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{tf_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=60.0, label='trace_filter query terminal',
        )
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{ts_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=30.0, label='timestamps query terminal',
        )
        ...
        wire_start = datetime.strptime(
            ts['startTime'].replace('Z', ''), '%Y-%m-%dT%H:%M:%S.%f',
        ...
        wire_end = datetime.strptime(
            ts['endTime'].replace('Z', ''), '%Y-%m-%dT%H:%M:%S.%f',
        ).replace(tzinfo=timezone.utc)
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{nm_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=30.0, label='no-match query terminal',
        )
        ...
        if mid['status'] != 'IN_PROGRESS':
            ...
            wait_until(
                lambda: http('GET', '/query_executions')[0] == 200,
                timeout=15.0, label='restarted server up',
            )
            # The recovered row.
        ...
        wait_until(
            lambda: http('GET', f'/query_executions/{ds_uuid}:status')[1]
                    ['status'] in ('SUCCESS', 'FAILED'),
            timeout=30.0, label='datasette test query terminal',
        )
