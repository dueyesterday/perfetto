...
def _qe_to_raw(snap: QESnapshot, truncate: bool = False) -> dict[str, Any]:
    ...
    if snap.table_name is not None:
        ...
        if CONFIG.datasette_port > 0:
            ...
            d['tableLink'] = (
                f'http://localhost:{CONFIG.datasette_port}/'
...
def _get_snapshot_or_404(uuid_str: str) -> QESnapshot:
    ...
    if snap is None:
        raise HTTPException(
            status_code=404, detail=f'Query {uuid_str} not found',
        )
    return snap
...
async def _run_async_query(
    ...
    try:
        ...
        await run_query_across_traces(
            POOL, traces, perfetto_sql, limit,
...
async def execute_async(request: Request) -> dict[str, Any]:
    ...
    db.insert_qe_async(new_uuid, perfetto_sql, limit)
    ...
    return _rows_response(['queryUuid'], [[new_uuid]])
...
async def execute_sync(request: Request) -> dict[str, Any]:
    ...
    Real BigTrace logs every execution — sync queries included — so the
    user sees their SQL + timing in the Ephemeral history tab. Sync
    queries are NOT materialized (rows aren't persisted to DuckDB); the
    history row records the metadata only. `:fetch_results` on a sync
    UUID returns 404 because tableName is NULL.
    """
    body = await request.json()
    perfetto_sql = body.get('perfetto_sql', '')
    limit = body.get('limit', 100)
    ...
    new_uuid = str(uuid.uuid4())
    # Capture submit time as UTC. We pass start_time/end_time as
    # explicit params to insert_qe_sync_terminal so the history row
    # reflects the actual run duration (not a single CURRENT_TIMESTAMP
    # that lands identical for both columns).
    start_time = utcnow()
    ...
    try:
        ...
        db.insert_qe_sync_terminal(
            new_uuid, perfetto_sql, limit,
            status='FAILED',
            processed_traces=0, total_traces=0, processed_rows=0,
            error_message=str(e.detail),
            start_time=start_time, end_time=utcnow(),
        )
        raise
    ...
    # Sync queries can't be cancelled by UUID (no uuid is exposed before
    # this returns). Still share a RunContext for consistency. Rows are
    # collected in-memory via `inline_rows` and returned inline.
    ctx = RunContext(global_limit=limit, inline_rows=[])
    cols, rows, err = await run_query_across_traces(
        POOL, traces, perfetto_sql, limit,
        max_concurrency=CONFIG.max_pool,
        ctx=ctx,
    )
    if err is not None:
        db.insert_qe_sync_terminal(
            new_uuid, perfetto_sql, limit,
            status='FAILED',
            processed_traces=ctx.processed_traces,
...
async def fetch_results(uuid: str, limit: int = 50, offset: int = 0) -> dict[str, Any]:
    ...
    result". When NULL we 404 — the UI's same-named guard rejects
    fetches proactively, and this is the safety net for races (e.g.
    TTL fires between the UI's check and the request).
    ...
    if snap.table_name is None:
        raise HTTPException(
            status_code=404,
            detail=(
                f'Materialized table for {uuid} is not available '
                '(query was sync, failed, cancelled with no rows, or its '
                'TTL expired)'
            ),
        )
...
async def cancel_query(uuid: str) -> Response:
    ...
    if snap is None:
        raise HTTPException(
            status_code=404, detail=f'Query {uuid} not found',
        )
...
async def list_executions() -> dict[str, Any]:
    ...
    return {
        'queryExecutions': [
            _qe_to_raw(s, truncate=True) for s in db.list_qes()
        ],
    }
...
def _patch_duckdb_for_plugin() -> None:
    ...
    def _patched_connect(database: str = ':memory:', *args, **kwargs):
        try:
            same_file = (
                database and CONFIG.db_path
                and os.path.abspath(database)
...
async def _run_datasette(port: int) -> None:
    ...
    metadata = {
        'plugins': {
            'datasette-parquet': {
                CONFIG.datasette_db_name: {'file': CONFIG.db_path},
            },
        },
    }
    ...
    log.info(
        ...
        port, CONFIG.datasette_db_name, CONFIG.db_path,
    )
    await server.serve()
...
async def _on_startup() -> None:
    ...
    if CONFIG.datasette_port > 0:
        _DATASETTE_TASK = asyncio.create_task(
            _run_datasette(CONFIG.datasette_port)
        )
    _TTL_TASK = asyncio.create_task(_ttl_sweep_loop())
    log.info(
        'BigTrace local TP backend ready: db=%s ttl=%ds sweep=%ds',
        CONFIG.db_path, CONFIG.table_ttl_seconds,
...
def main() -> None:
    ...
    parser.add_argument(
        '--table-ttl-seconds', type=int, default=86400,
    ...
    parser.add_argument(
        '--table-ttl-sweep-seconds', type=int, default=300,
        help='How often the TTL sweep runs. Default 5 minutes.')
    parser.add_argument(
        '--with-db-ui', dest='db_ui_port', type=int, default=0, nargs='?',
        const=4213,
    ...
    parser.add_argument(
        '--with-datasette', dest='datasette_port', type=int, default=0,
        nargs='?', const=8003,
